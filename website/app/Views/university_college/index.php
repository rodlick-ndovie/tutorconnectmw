    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5 mb-8">
            <div class="max-w-3xl">
                ...
                <h2 class="mt-2 text-3xl font-extrabold text-secondary">University experts ready to support academic work</h2>
                <p class="mt-3 text-slate-600 leading-7">Browse approved university and college support experts and firms by service area, teaching mode, and location.</p>
            </div>
        ...
        <?php if (!empty($approvedTutors)): ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($approvedTutors as $tutor): ?>
                    ...
                    ?>
                    <article class="h-full rounded-lg border border-slate-200 bg-white shadow-sm hover:border-primary transition overflow-hidden flex flex-col">
                        <div class="h-36 sm:h-40 bg-gradient-to-br from-orange-50 to-slate-100 flex items-center justify-center overflow-hidden border-b border-slate-100">
                            <?php if ($profilePicture !== ''): ?>
                                <img src="<?= base_url($profilePicture) ?>" alt="<?= esc($displayName) ?>" class="w-full h-full <?= $isFirm ? 'object-contain p-4' : 'object-cover' ?>" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div class="hidden w-full h-full items-center justify-center text-4xl font-extrabold text-primary"><?= esc($initials) ?></div>
                            <?php else: ?>
                                <div class="w-full h-full flex items-center justify-center text-4xl font-extrabold text-primary"><?= esc($initials) ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="p-4 flex-1">
                            <div class="mb-4">
                                <h3 class="text-lg font-bold text-slate-900 leading-snug"><?= esc($displayName) ?></h3>
                            </div>

                            <div class="grid gap-2 text-sm text-slate-600 mb-4">
                                ...
                                <div class="flex items-start gap-2">
                                    ...
                                    <span class="leading-6"><?= esc(!empty($institutions) ? implode(', ', array_slice($institutions, 0, 2)) : ($isFirm ? 'Business background not listed' : 'Institution not listed')) ?></span>
                                </div>
                            </div>
                        ...
                        <div class="border-t border-slate-100 bg-slate-50/70 p-4">
                            <a href="<?= site_url('university-tutor/' . (int) ($tutor['id'] ?? 0)) ?>" class="inline-flex items-center justify-center w-full rounded-md bg-secondary text-white text-sm font-semibold px-4 py-2.5 hover:bg-primary transition">
                                <?= $isFirm ? 'View Firm Profile' : 'View Academic Profile' ?> <i class="fas fa-arrow-right ml-2"></i>
                            </a>
                        </div>
                    </article>
                <?php endforeach; ?>
...
<section class="bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div class="grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-8">
            <div>
                <div class="text-sm font-bold text-primary uppercase tracking-wide">For Experts and Firms</div>
                ...
                <p class="mt-3 text-slate-600 leading-7">
                    University experts and firms submit background details, service areas, documents, availability, and pricing before admin approval.
                </p>
                <div class="mt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="rounded-lg border border-gray-200 bg-gray-50 p-5">
                    ...
                    <ul class="space-y-2 text-sm text-slate-700">
                        <li>Profile picture or company logo</li>
                        <li>National ID or business registration certificate</li>
                        <li>Academic, business, or professional proof</li>
                        <li>Institutions, firm background, or current work</li>
