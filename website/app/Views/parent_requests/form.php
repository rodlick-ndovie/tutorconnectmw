...
<?php
    ...
    $requesterRole = old('requester_role', 'parent');
    $requesterRole = in_array($requesterRole, ['parent', 'learner', 'teacher'], true) ? $requesterRole : 'parent';
?>
...
<style>
    ...
    .choice-card.is-active {
        ...
        box-shadow: 0 0 0 3px rgba(229, 92, 13, 0.1), 0 10px 24px rgba(15, 23, 42, 0.05);
    }
...
<section class="bg-gray-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <section class="selector-panel mb-6">
            <div class="mb-4">
                <h2 class="text-lg font-extrabold text-secondary">Who is making this request?</h2>
                <p class="mt-1 text-sm text-gray-600">This page is for parents and learners who need academic support.</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-3" role="radiogroup" aria-label="Requester type">
                <label class="requester-card choice-card <?= $requesterRole === 'parent' ? 'is-active' : '' ?>">
                    <input type="radio" name="requester_role_selector" value="parent" class="sr-only requester-role-input" <?= $requesterRole === 'parent' ? 'checked' : '' ?>>
                    <span class="choice-check"><i class="fas fa-check"></i></span>
                    <span class="choice-head"><span class="choice-icon"><i class="fas fa-user-group"></i></span>Parent / Guardian</span>
                    <span class="choice-copy">I am requesting support for a child or dependent learner.</span>
                </label>
                <label class="requester-card choice-card <?= $requesterRole === 'learner' ? 'is-active' : '' ?>">
                    <input type="radio" name="requester_role_selector" value="learner" class="sr-only requester-role-input" <?= $requesterRole === 'learner' ? 'checked' : '' ?>>
                    <span class="choice-check"><i class="fas fa-check"></i></span>
                    <span class="choice-head"><span class="choice-icon"><i class="fas fa-book-open"></i></span>Learner / Student</span>
                    <span class="choice-copy">I am looking for a teacher or specialist for myself.</span>
                </label>
                <label class="requester-card choice-card <?= $requesterRole === 'teacher' ? 'is-active' : '' ?>">
                    <input type="radio" name="requester_role_selector" value="teacher" class="sr-only requester-role-input" <?= $requesterRole === 'teacher' ? 'checked' : '' ?>>
                    <span class="choice-check"><i class="fas fa-check"></i></span>
                    <span class="choice-head"><span class="choice-icon"><i class="fas fa-chalkboard-teacher"></i></span>Teacher / Expert</span>
                    <span class="choice-copy">I want to offer teaching or university expert support.</span>
                </label>
            </div>

            <div id="teacherGuidanceAlert" class="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 <?= $requesterRole === 'teacher' ? '' : 'hidden' ?>">
                <div class="font-bold text-amber-950">This request page is for parents and learners.</div>
                <p class="mt-2 leading-6">If you want to offer services, create the correct provider account instead.</p>
                <div class="mt-4 flex flex-col sm:flex-row gap-3">
                    <a href="<?= site_url('university-college-support/register') ?>" class="inline-flex items-center justify-center rounded-lg bg-primary px-4 py-2.5 text-white font-bold hover:bg-red-600 transition">
                        Uni expert support account <i class="fas fa-arrow-right ml-2"></i>
                    </a>
                    <a href="<?= site_url('register') ?>" class="inline-flex items-center justify-center rounded-lg border border-primary bg-white px-4 py-2.5 text-primary font-bold hover:bg-primary hover:text-white transition">
                        Regular tutor account <i class="fas fa-arrow-right ml-2"></i>
                    </a>
                </div>
            </div>
        </section>

        <div id="requestFormsWrap">
        <section class="selector-panel mb-6">
            <div class="mb-4">
                <h2 class="text-lg font-extrabold text-secondary">What kind of tutor support do you need?</h2>
                <p class="mt-1 text-sm text-gray-600">Choose the matching path so the request reaches the right providers.</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" role="tablist" aria-label="Request type">
                <button type="button" data-request-tab="school" class="request-tab choice-card text-left <?= $activeType === 'school' ? 'is-active' : '' ?>">
                    <span class="choice-check"><i class="fas fa-check"></i></span>
                    <span class="choice-head"><span class="choice-icon"><i class="fas fa-chalkboard-teacher"></i></span>School Teacher</span>
                    <span class="choice-copy">For primary, secondary, curriculum, grade/class, and subject-based tutoring.</span>
                </button>
                <button type="button" data-request-tab="university" class="request-tab choice-card text-left <?= $activeType === 'university' ? 'is-active' : '' ?>">
                    <span class="choice-check"><i class="fas fa-check"></i></span>
                    <span class="choice-head"><span class="choice-icon"><i class="fas fa-user-graduate"></i></span>University Specialist</span>
                    <span class="choice-copy">For college, dissertation, ICT, statistics, research, finance, and academic consultations.</span>
                </button>
            </div>
        </section>

        <div id="requestTypeMessage" class="request-type-message"></div>
        ...
        <div id="schoolRequestPanel" class="<?= $activeType === 'school' ? '' : 'hidden' ?>">
            ...
            <form method="post" action="<?= site_url('request-tutor') ?>" class="bg-white border border-gray-200 rounded-lg shadow-sm p-5 sm:p-7 space-y-6">
                ...
                <input type="hidden" name="requester_role" value="<?= esc($requesterRole) ?>" data-requester-role-field>
...
<script>
    const requesterRoleInputs = document.querySelectorAll('.requester-role-input');
    const requesterCards = document.querySelectorAll('.requester-card');
    const teacherGuidanceAlert = document.getElementById('teacherGuidanceAlert');
    const requestFormsWrap = document.getElementById('requestFormsWrap');
    ...
    function setRequestType(type) {
        ...
        const message = document.getElementById('requestTypeMessage');
        if (message) {
            message.innerHTML = isUniversity
                ? '<strong><i class="fas fa-circle-info"></i>University specialist request</strong><span>Use this path when the learner needs college or university-level academic support, such as research guidance, dissertation planning, programming, statistics, finance, ICT, or specialist consultations.</span>'
                : '<strong><i class="fas fa-circle-info"></i>School teacher request</strong><span>Use this path when the learner needs primary or secondary school tutoring based on curriculum, grade/class, subjects, district, and preferred lesson mode.</span>';
        }
    ...
    function syncRequesterRole() {
        const selected = document.querySelector('.requester-role-input:checked');
        const role = selected ? selected.value : 'parent';
        const isTeacher = role === 'teacher';

        requesterCards.forEach((card) => {
            const input = card.querySelector('.requester-role-input');
            const active = input && input.checked;
            card.classList.toggle('is-active', active);
        });

        if (teacherGuidanceAlert) {
            teacherGuidanceAlert.classList.toggle('hidden', !isTeacher);
        }

        if (requestFormsWrap) {
            requestFormsWrap.classList.toggle('opacity-50', isTeacher);
            requestFormsWrap.classList.toggle('pointer-events-none', isTeacher);
        }

        document.querySelectorAll('[data-requester-role-field]').forEach((field) => {
            field.value = role;
        });

        document.querySelectorAll('#requestFormsWrap input, #requestFormsWrap select, #requestFormsWrap textarea, #requestFormsWrap button').forEach((field) => {
            if (field.hasAttribute('data-request-tab')) {
                return;
            }
    ...
    requesterRoleInputs.forEach((input) => {
        input.addEventListener('change', syncRequesterRole);
    });

    syncRequesterRole();
    setRequestType(<?= json_encode($activeType) ?>);
