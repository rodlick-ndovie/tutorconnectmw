...
$isFirmAccount = !empty($is_firm_account);
$profileDisplayName = trim((string) ($profile['full_name'] ?? ''));
$contactDisplayName = trim((string) (($user['first_name'] ?? '') . ' ' . ($user['last_name'] ?? '')));
$portalDisplayName = $isFirmAccount ? $profileDisplayName : $contactDisplayName;
if ($portalDisplayName === '') {
    $portalDisplayName = $profileDisplayName !== '' ? $profileDisplayName : $contactDisplayName;
}
...
<body>
    <?= view('university_portal/partials/shell_start', [
        'nav_title' => 'Premium',
        'nav_subtitle' => $isFirmAccount ? 'Firm Subscription Plan' : 'University Subscription Plans',
        'nav_user' => $portalDisplayName,
        'show_home_shortcut' => true,
    ]) ?>
    <div class="page">
        <section class="hero">
            <h1 style="margin:0 0 8px;"><?= $isFirmAccount ? 'Firm Subscription Plan' : 'University Subscription Plans' ?></h1>
            <p style="margin:0; color:var(--text-muted); line-height:1.6;"><?= $isFirmAccount ? 'Firms use one fixed university portal plan at MWK 15,000 per month. Continue below through the shared subscription checkout flow.' : 'This uses the same payment logic as the tutor portal. Choose a plan below and continue through the shared subscription checkout flow.' ?></p>
