...
<html lang="en">
<?php
$resultStatus = (string) ($status ?? 'success');
$storedPaymentStatus = (string) ($subscription['payment_status'] ?? '');
$storedSubscriptionStatus = (string) ($subscription['status'] ?? '');
$isConfirmed = $resultStatus === 'success' && $storedPaymentStatus === 'verified' && $storedSubscriptionStatus === 'active';
$pageSubtitle = $isConfirmed ? 'Payment Successful' : 'Payment Processing';
$pageTitle = $isConfirmed ? 'Payment Successful' : 'Payment Confirmation Pending';
$iconClass = $isConfirmed ? 'fa-check' : 'fa-clock';
$iconToneClass = $isConfirmed ? 'icon-success' : 'icon-processing';
$statusLabel = $isConfirmed ? 'Active' : 'Pending confirmation';
?>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= esc($pageSubtitle) ?> - University Portal</title>
...
<body>
        ...
        <section class="card">
            ...
            <?php if ($subscription): ?>
                <div class="details">
                    ...
                    <?php endif; ?>
                    <?php if ($isConfirmed && !empty($subscription['current_period_end'])): ?>
                        <div class="row">
    ...
    <?php if (!empty($enablePolling) && !empty($txRef)): ?>
        <script>
            let pollAttempts = 0;
