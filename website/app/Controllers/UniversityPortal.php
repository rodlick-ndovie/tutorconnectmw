...
class UniversityPortal extends BaseController
{
    ...
    public function saveProfile()
    {
        ...
        if (!$hasExistingProfilePicture && !$this->isValidUpload($profilePicture)) {
            $errors['profile_picture'] = $isFirm ? 'Company logo is required.' : 'Profile picture is required.';
        }

        if ($this->isValidUpload($profilePicture) && !in_array($this->getUploadedExtension($profilePicture), ['jpg', 'jpeg', 'png'], true)) {
            $errors['profile_picture'] = $isFirm ? 'Company logo must be a JPG or PNG image.' : 'Profile picture must be a JPG or PNG image.';
        }

        if (!$hasExistingNationalId && !$this->isValidUpload($nationalIdFile)) {
            $errors['national_id_file'] = $isFirm ? 'Business registration certificate is required.' : 'National ID file is required.';
        }

        if ($this->isValidUpload($nationalIdFile) && !in_array($this->getUploadedExtension($nationalIdFile), ['jpg', 'jpeg', 'png', 'pdf'], true)) {
            $errors['national_id_file'] = $isFirm ? 'Business registration certificate must be an image or PDF file.' : 'National ID must be an image or PDF file.';
        }
            ...
            foreach ($certificationFiles as $file) {
                if ($this->isValidUpload($file)) {
                    if (!in_array($this->getUploadedExtension($file), ['jpg', 'jpeg', 'png', 'pdf'], true)) {
                        $errors['certification_files'] = $isFirm ? 'Business certificate files must be image or PDF files.' : 'Certification files must be image or PDF files.';
                        continue;
                    }
        ...
        if ($existingCertificationPaths === [] && $validCertificationFiles === []) {
            $errors['certification_files'] = $isFirm ? 'Upload at least one supporting business or professional certificate.' : 'Upload at least one academic transcript or certification file.';
        }
    ...
    public function subscription()
    {
        ...
        $availablePlans = $this->filterUniversityPlansForProfile($availablePlans, $profile);
        ...
        return view('university_portal/subscription', [
            ...
            'user' => $user,
            'profile' => $profile,
            'is_firm_account' => ($profile['account_type'] ?? 'individual') === 'firm',
    ...
    private function filterUniversityPlansForProfile(array $plans, array $profile): array
    {
        $isFirm = ($profile['account_type'] ?? 'individual') === 'firm';

        return array_values(array_filter($plans, static function (array $plan) use ($isFirm): bool {
            $isFirmPlan = strcasecmp(trim((string) ($plan['name'] ?? '')), 'Firm') === 0;

            return $isFirm ? $isFirmPlan : !$isFirmPlan;
        }));
    }
