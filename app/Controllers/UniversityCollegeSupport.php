...
class UniversityCollegeSupport extends BaseController
{
    ...
    public function registerStep1()
    {
        ...
        if ($data['year_of_study_or_graduation'] === '') {
            $errors['year_of_study_or_graduation'] = 'Year of study or graduation is required';
        }
        if ($data['bio'] === '' || mb_strlen($data['bio']) < 40) {
            $errors['bio'] = 'Bio is required and must be at least 40 characters';
        }
    ...
    public function registerStep2()
    {
        ...
        $fullName = trim(($completeData['first_name'] ?? '') . ' ' . ($completeData['last_name'] ?? ''));
        ...
        $isEmployed = ($completeData['work_status'] ?? '') === 'Employed' ? 1 : 0;
        ...
        $userData = [
            ...
            'school_name' => $completeData['employer_name'] ?: null,
            'role' => 'trainer',
            'is_active' => 0,
        ...
        try {
            ...
            $saved = $universityTutorModel->insert([
                ...
                'work_status' => trim((string) ($completeData['work_status'] ?? '')) ?: null,
                'employer_name' => trim((string) ($completeData['employer_name'] ?? '')) ?: null,
                'employer_contact' => trim((string) ($completeData['employer_contact'] ?? '')) ?: null,
    ...
    private function getApprovedUniversityTutors(int $limit = 12): array
    {
        ...
        $tutors = \Config\Database::connect()
            ->table('university_college_tutors uct')
            ->select('uct.id, uct.full_name, uct.profile_picture, uct.institutions_json, uct.service_areas_json, uct.teaching_mode, uct.city_location, uct.status, uct.updated_at, uct.created_at, sp.name AS plan_name, sp.search_ranking, sp.badge_level')
            ->join('tutor_subscriptions ts', 'ts.user_id = uct.user_id', 'inner')
            ->join('subscription_plans sp', 'sp.id = ts.plan_id', 'left')
            ->where('uct.status', 'approved')
            ->where('ts.status', 'active')
